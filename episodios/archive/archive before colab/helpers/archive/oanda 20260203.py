...
base_folder = 'C:\\Users\\BernardoElli_P\\OneDrive\\Python\\Git\\Phi\\OOP'
...
# OANDA_ENV = os.getenv("OANDA_ENV", "demo").lower()
...
def get_environmental_parameters(environment):
    ...
    # base_folder = os.getcwd()  # Works in Spyder   
...
# def read_credentials_Oanda(environment):
...
# account_id, access_token = read_credentials_Oanda()
...
# def place_simple_order(environment, instrument, volume):
#     base_url, credentials_file_name, account_id, access_token = get_environmental_parameters(environment)
#     #place_market_order(access_token, account_id, instrument, units, stop_loss, take_profit) 
#     endpoint = f'{base_url}/v3/accounts/{account_id}/orders'
#     # headers
...
#         trade_id = (
#             response_json.get("orderFillTransaction", {})
#             .get("tradeOpened", {})
#             .get("tradeID")
#         )

#         action = "Buying" if volume > 0 else "Selling"
#         print(f'{action} {instrument}. Trade ID: {trade_id}')
#         return trade_id  # ⬅️ Return this so you can track later
#     else:
#         print(f">>> Error placing order. Status code: {response.status_code}, Response: {response.text}")
#         return None 
    
# def get_candle_exec_time(seconds_before_new_hour): # get_candle_exec_time(15)
#     # Get current UTC time
#     current_time_utc = datetime.utcnow()
#     # Calculate the start of the next hour
#     next_hour_start = current_time_utc.replace(minute=0, second=0, microsecond=0) + timedelta(hours=1)
#     # Subtract 15 seconds from the start of the next hour
#     next_hour_start_minus_15 = next_hour_start - timedelta(seconds=seconds_before_new_hour)
#     # Format the result as 'YYYY-MM-DDTHH:MM:SSZ'
#     formatted_time = next_hour_start_minus_15.strftime('%Y-%m-%dT%H:%M:%SZ')
...
def get_table(environment, price):     
    ...
    utc_time, local_time, _ = of.get_utc_and_local_time()    
    print(f'\n---> Starting process: get_updated_table() - UTC:{utc_time} (LT:{local_time}) ')
    # read instruments list
    instruments_folder = 'C:\\Users\\BernardoElli\\OneDrive\\Python\\Git\\Phi\\files'   
    ...
    updated_table = of.adjust_JPY_pairs(updated_table)
    updated_table = of.adjust_volume(updated_table)
...
def get_last_hour_bid_ask(environment, instrument):
    ...
    candle_time = last_hour.strftime('%Y-%m-%dT%H:%M:%SZ')
    ...
    return {
        "time": candle_time,
        "bid_close": float(bid_candle["bid"]["c"]),
        "ask_close": float(ask_candle["ask"]["c"]),
    }
...
def place_entry_order_with_offset(environment, instrument, volume, 
    ...
    is_buy = volume > 0
    prices = get_last_hour_bid_ask(environment, instrument)    
    if is_buy:
    ...
    expiry_time = (datetime.utcnow() + timedelta(hours=1)).replace(tzinfo=timezone.utc).isoformat().replace('+00:00', 'Z')
    ...
    order_data = {
        "order": {
            ...
            "clientExtensions": {
              "tag": tag,
              "comment": f"offset={offset_points},tp={tp_points},sl={sl_points},"
            },
            "takeProfitOnFill": {
                "price": tp_price
            },
    ...
    if response.status_code == 201:
        response_json = response.json()
        order_id = response_json.get("orderCreateTransaction", {}).get("id")
        print(f"✅ LIMIT order placed: {'BUY' if is_buy else 'SELL'} {instrument} at {entry_price}")
...
def get_candle_at(environment, instrument, candle_datetime, price_type, granularity = 'H1' ):
  # get_candle_at('demo','AUD_CAD',pd.to_datetime('2025-01-02 04:00:00.000').strftime('%Y-%m-%dT%H:%M:%SZ'),'a') 
    base_url, credentials_file_name, account_id, access_token = get_environmental_parameters(environment)
